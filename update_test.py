...
def weighted_peak(data, peak_threshold, prominence_threshold, width_threshold, weight):
    # Make a copy of the data to avoid modifying the original
    weighted_data = data.copy()
    
    # Find all peak points and their properties
    peaks, properties = find_peaks(weighted_data, prominence=prominence_threshold, height=peak_threshold)
    
    # Get prominences and widths for the peaks
    prominences = properties["prominences"]
    widths, _, _, _ = peak_widths(weighted_data, peaks)
    
    for i, peak in enumerate(peaks):
        if (weighted_data[peak] <= peak_threshold and 
            prominences[i] <= prominence_threshold and 
            widths[i] <= width_threshold):
...
def preprocess_and_train_for_category(dataset, category, window_size, epochs, weight, weighting_func, 
                                      section_threshold=None, peak_threshold=None, 
                                      prominence_threshold=None, width_threshold=None):
    ...
    if weighting_func == "weighted_peak":
        operation_data = weighted_peak(data_subset, peak_threshold, prominence_threshold, width_threshold, weight)
...
def preprocess_and_train(dataset, category, window_size, epochs, weight, weighting_func, 
                         section_threshold=None, peak_threshold=None, 
                         prominence_threshold=None, width_threshold=None):
    dataset = dataset.dropna(subset=['Category'])
    
    if category == 'EU':
        lstm_model, model_dif = preprocess_and_train_for_category(dataset, 'EU', window_size, epochs, weight, weighting_func, 
                                                                  section_threshold, peak_threshold, prominence_threshold, width_threshold)
    elif category == 'US':
        lstm_model, model_dif = preprocess_and_train_for_category(dataset, 'US', window_size, epochs, weight, weighting_func, 
                                                                  section_threshold, peak_threshold, prominence_threshold, width_threshold)
    else:
...
def real_time_visualization(real_data, model, model_dif, window_size, weight, buffer_size, 
                            weighting_func, section_threshold=None, peak_threshold=None, 
                            prominence_threshold=None, width_threshold=None):
    ...
    # Choosing the weighting function based on the user's selection
    if weighting_func == "weighted_peak":
        weighted_data = weighted_peak(raw_data_values, peak_threshold, weight, prominence_threshold, width_threshold)
    elif weighting_func == "weighted_section":
...
def run_visualization(real_data, window_size, section_threshold, weight, buffer_size, 
                      peak_threshold, prominence_threshold=None, width_threshold=None):
    ...
    for category in unique_categories:
        ...
        windows, predictions, anomaly_scores, errors = real_time_visualization(
            data_subset, lstm_model, model_dif, window_size, weight, buffer_size, 
            "weighted_peak", section_threshold, peak_threshold, prominence_threshold, width_threshold)
...
def app():
    ...
    for category in ['EU', 'US']:
        ...
        # First subheader with peak, prominence, and width threshold values
        st.subheader(f"{category} Peak Control")
        if weighting_func == "weighted_peak":
            ...
            st.session_state[f"{category}_prominence_threshold"] = st.number_input(f'Enter prominence threshold value for {category}', value=5.6, step=0.01)
            st.session_state[f"{category}_width_threshold"] = st.number_input(f'Enter width threshold value for {category}', value=2.43, step=0.01)

        # Separate subheader for section threshold value
        elif weighting_func == "weighted_section":
            st.subheader(f"{category} Section Control")
        ...
        # Separate subheader for weight value
        st.subheader(f"{category} Weight Control")
    ...
    if st.button("Train LSTM Models for EU & US"):
        ...
        for index, category in enumerate(['EU', 'US']):
            ...
            if st.session_state["weighting_func"] == "weighted_peak":
                ...
                category_prominence_threshold = st.session_state.get(f"{category}_prominence_threshold", None)
                category_width_threshold = st.session_state.get(f"{category}_width_threshold", None)
                
                if category_peak_threshold and category_prominence_threshold and category_width_threshold:
                    weighted_data = weighted_peak(category_dataset['VALUE'].values, category_peak_threshold, category_weight, category_prominence_threshold, category_width_threshold)
            ...
            progress_bar.progress((index + 1) * 0.5)
            ...
            with st.spinner("Running real-time visualizations..."):
                ...
                for category in ['EU', 'US']:
                    if category not in categories_processed:
                        ...
                        if st.session_state["weighting_func"] == "weighted_peak":
                            if not category_peak_threshold or not category_weight:
                                st.error(f"Missing values for {category}. Please ensure all required inputs are provided.")
                                return
                            weighted_data = weighted_peak(category_dataset['VALUE'].values, category_peak_threshold, category_weight)
                        else:
                            if not category_section_threshold or not category_weight:
                                st.error(f"Missing values for {category}. Please ensure all required inputs are provided.")
                                return
                            weighted_data = weighted_section(category_dataset['VALUE'].values, category_section_threshold, category_weight)
                        ...
                        st.info(f"Processed data for {category}")
...
def find_peak_indices_above_threshold(data, value_threshold, prominence_threshold, width_threshold):
    peaks, properties = find_peaks(data, prominence=prominence_threshold, width=width_threshold)
    
    # Filter peaks based on VALUE and returned properties
    valid_peaks = [peak for peak in peaks if data[peak] > value_threshold and properties['prominences'][peak] > prominence_threshold and properties['widths'][peak] > width_threshold]
...
def check_anomal_point():
    ...
    # Inputs for peak's VALUE, Prominence, and Width thresholds
    peak_value_threshold = st.number_input(f'Enter peak VALUE threshold for {category}', value=0.2, step=0.001)
    peak_prominence_threshold = st.number_input(f'Enter peak Prominence threshold for {category}', value=0.1, step=0.001)
    peak_width_threshold = st.number_input(f'Enter peak Width threshold for {category}', value=5, step=1)
    ...
    if st.button("Check Thresholds"):
        ...
        # Check against the peak's VALUE, Prominence, and Width thresholds
        peak_indices = find_peak_indices_above_threshold(weighted_data, peak_value_threshold, peak_prominence_threshold, peak_width_threshold)
        st.write(f"Indices where peak VALUE, Prominence, and Width exceed thresholds: {peak_indices}")
